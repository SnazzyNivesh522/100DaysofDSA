...
public class MinimumFallingPathSum1 {
    private int fMPS(int row,int col,int [][]matrix){
        ...
        int diagleft=fMPS(row+1,col-1,matrix);
        int diagright=fMPS(row+1,col+1,matrix);
        int below=fMPS(row+1,col,matrix);

        int minPath=Math.min(diagleft,diagright);
        minPath=Math.min(minPath,below);
        return matrix[row][col]+minPath;
    }
    public int minFallingPathSum(int[][] matrix) {
        int minfallingsum=Integer.MAX_VALUE;
        for(int c=0;c<matrix[0].length;c++){
            minfallingsum=Math.min(minfallingsum,fMPS(0,c,matrix));
        }
        return minfallingsum;
    }
    public static void main(String[] args) {
        System.out.println(new MinimumFallingPathSum1().minFallingPathSum(new int[][]{{2,1,3},{6,5,4},{7,8,9}}));
        System.out.println(new MinimumFallingPathSum1().minFallingPathSum(new int[][]{{-19,57},{-40,-5}}));
        System.out.println(new MinimumFallingPathSum1().minFallingPathSum(new int[][]{{-19, -1, -96, 48, -94, 36, 16, 55, -42, 37, -59, 6, -32, 96, 95, -58, 13, -34, 94, 85},
