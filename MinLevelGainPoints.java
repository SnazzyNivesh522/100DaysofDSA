 ...
 */
public class MinLevelGainPoints {
    public int minimumLevels(int[] possible) {
        int n = possible.length;
        int danielPoints[]=new int[n-1];
        int bobPoints[]=new int[n-1];
        if(possible[0]==0){
            danielPoints[0]=-1;
        }
        else{
        ...
        for (int i = 1; i < n-1; i++) {
            if(possible[i]==0){
                danielPoints[i]=danielPoints[i-1]-1;
            }
            else{
                danielPoints[i]=danielPoints[i-1]+1;                
            }
        ...
        for(int i=0;i<n-1;i++){
            ...
            if(bobPoints[i]<danielPoints[i]) return i+1;
        }
